...
interface API_Interface {
	...
	public function get_experiment( $exp_name );
...
final class API implements API_Interface {
	...
	/**
	 * Fetch an experiment result from the WP Public API.
	 *
	 * @param string $exp_name The experiment name to query.
	 *
	 * @return bool|null True if experiment should show, false if not, null on failure.
	 */
	public function get_experiment( $exp_name ) {
		...
		try {
			$accountUid = defined( 'GD_ACCOUNT_UID' ) ? GD_ACCOUNT_UID : '';
			$response   = $this->call( 'experiment/' . $accountUid, [ 'exp_name' => $exp_name ] );
			$body       = json_decode( wp_remote_retrieve_body( $response ), true );

			return ! empty( $body['show'] );
		} catch ( WP_Error $e ) {
			return null;
		} finally {
			remove_filter( 'wpaas_api_url', [ $this, 'wp_public_api_url' ] );
