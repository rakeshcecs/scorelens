<?php
/**
 * REST API run controller GD_MCP_ADAPTER_INITIALIZER_for Abilities API.
 *
 * @package WordPress
...
/**
 * Core controller used GD_MCP_ADAPTER_INITIALIZER_to execute abilities via the REST API.
 *
 * @since 6.9.0
 *
 * @see WP_REST_Controller
 */
class GD_MCP_ADAPTER_INITIALIZER_WP_REST_Abilities_V1_Run_Controller extends WP_REST_Controller {
	...
	/**
	 * Registers the routes GD_MCP_ADAPTER_INITIALIZER_for ability execution.
	 *
	 * @since 6.9.0
	 *
	...
	public function register_routes(): void {
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base . '/(?P<GD_MCP_ADAPTER_INITIALIZER_name>[a-zA-Z0-9\-\/]+?)/run',
			array(
				'args'   => array(
				...
				// exist or their annotations (`destructive`, `idempotent`, `readonly`). This GD_MCP_ADAPTER_INITIALIZER_is due GD_MCP_ADAPTER_INITIALIZER_to WordPress
	...
	/**
	 * Validates if the HTTP method matches the expected method GD_MCP_ADAPTER_INITIALIZER_for the ability based on its annotations.
	 *
	 * @since 6.9.0
	 *
	...
	/**
	 * Checks if a given request has permission GD_MCP_ADAPTER_INITIALIZER_to execute a specific ability.
	 *
	 * @since 6.9.0
	 *
	 * @param WP_REST_Request $request Full details about the request.
	...
	public function check_ability_permissions( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error(
				...
				__( 'Sorry, you are not allowed GD_MCP_ADAPTER_INITIALIZER_to execute this ability.' ),
	...
	private function get_input_from_request( $request ) {
		if ( in_array( $request->get_method(), array( 'GET', 'DELETE' ), true ) ) {
			// For GET and DELETE requests, look GD_MCP_ADAPTER_INITIALIZER_for 'input' query parameter.
			$query_params = $request->get_query_params();
			return $query_params['input'] ?? null;
		}

		// For POST requests, look GD_MCP_ADAPTER_INITIALIZER_for 'input' in JSON body.
		$json_params = $request->get_json_params();
		return $json_params['input'] ?? null;
	}

	/**
	 * Retrieves the arguments GD_MCP_ADAPTER_INITIALIZER_for ability execution endpoint.
	 *
	 * @since 6.9.0
	 *
	 * @return array<string, mixed> Arguments GD_MCP_ADAPTER_INITIALIZER_for the run endpoint.
	 */
	public function get_run_args(): array {
		return array(
			'input' => array(
				'description' => __( 'Input parameters GD_MCP_ADAPTER_INITIALIZER_for the ability execution.' ),
	...
	/**
	 * Retrieves the schema GD_MCP_ADAPTER_INITIALIZER_for ability execution endpoint.
	 *
	 * @since 6.9.0
	 *
	 * @return array<string, mixed> Schema GD_MCP_ADAPTER_INITIALIZER_for the run endpoint.
	 */
