...
class MetricsItem implements EnvelopeItemInterface
{
    ...
    public static function toEnvelopeItem(Event $event): string
    {
        ...
        if (!empty($metricMetaPayload)) {
            $metricMetaPayload = JSON::encode(['timestamp' => time(), 'mapping' => $metricMetaPayload]);
            $metricMetaHeader = ['type' => 'metric_meta', 'length' => mb_strlen($metricMetaPayload)];
            return sprintf("%s\n%s\n%s\n%s", JSON::encode($statsdHeader), $statsdPayload, JSON::encode($metricMetaHeader), $metricMetaPayload);
        }
        return sprintf("%s\n%s", JSON::encode($statsdHeader), $statsdPayload);
    }
}
