...
class TypeHelper
{
    ...
    /**
     * Strips out any array elements that are not strings or that don't have a key with a string type and returns the array.
     *
     * @param mixed $array
     * @return array<string, string>
     */
    public static function arrayOfStringsWithStringsAsKeys($array) : array
    {
        return array_filter(TypeHelper::array($array, []), static function ($value, $key) {
            return is_string($key) && is_string($value);
        }, ARRAY_FILTER_USE_BOTH);
    }

    /**
     * Strips out any array elements that are not arrays of strings with strings as keys and returns the array.
     *
     * @param mixed $array
     * @return array<string, array<string, string>>
     */
    public static function arrayOfArraysOfStringsWithStringsAsKeys($array) : array
    {
        $result = [];

        foreach (TypeHelper::array($array, []) as $key => $value) {
            if (! is_string($key)) {
                continue;
            }

            $filteredValue = static::arrayOfStringsWithStringsAsKeys($value);
